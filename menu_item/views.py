...
def menu(restaurant_id):
    ...
    return render_template('menu_item/menu_item.html',
...
def new_menu(restaurant_id):
        ...
        if restaurant.user == login_session['username']:
            if request.method == 'POST':
                if form.validate_on_submit():
                    ...
                    menu_item = Menu_items(form.course.data,
                                           form.item_name.data,
                                           form.description.data,
                                           form.price.data, filename,
    ...
    return render_template('menu_item/new_menu.html',
                           restaurant_id=restaurant_id,
                           form=form)
...
def menu_edit(restaurant_id, menu_id):
    form = New_menu()
    menu_item = Menu_items.query.filter_by(id=menu_id,
                                           restaurant_id=restaurant_id).first()  # NOQA
    if 'username' in login_session:
        restaurant = Restaurant_name.query.filter_by(id=restaurant_id).first()
        if restaurant.user == login_session['username']:
            if request.method == 'POST':
                if form.validate_on_submit():
                    ...
                    db.session.add(menu_item)
                    db.session.commit()
                    flash("Menu Item " + menu_item.menu_name +
    ...
    return render_template('menu_item/menu_edit.html',
                           form=form,
