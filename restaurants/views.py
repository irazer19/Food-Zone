...
from flask import render_template, url_for, redirect, request, flash, session as login_session, make_response, abort
...
def restaurants():
	...
	return render_template('restaurants/restaurants.html', all_names=all_names)

@app.route('/myrestaurants')
def my_restaurants():
	myrestaurants = Restaurant_name.query.filter_by(user=login_session['username']).all()
	return render_template('restaurants/my_restaurants.html', myrestaurants=myrestaurants)
	...
	if 'username' in login_session:
		...
		if request.method == 'POST':
			if form.validate_on_submit():
				restaurant_name = Restaurant_name(form.restaurant_name.data, login_session['username'])
...
def edit(restaurant_id):
	form = New_restaurantForm()
	restaurant_name = Restaurant_name.query.filter_by(id=restaurant_id).first()
	if 'username' in login_session:
		...
		return redirect(url_for('login'))
	return render_template('restaurants/edit.html', form=form, value=restaurant_name.name, restaurant_id=restaurant_id)
...
def delete(restaurant_id):
	if 'username' in login_session:
		restaurant = Restaurant_name.query.filter_by(id=restaurant_id).first()
		if restaurant.user == login_session['username']:
			menu_item = Menu_items.query.filter_by(restaurant_id=restaurant_id).delete()
			restaurant = Restaurant_name.query.filter_by(id=restaurant_id).first()
